"""Expert Mode Selection Handlers - manages user authentication and language selection."""
...
def render_pro_authentication():
    """Render authentication form for Expert mode. Returns (is_authenticated, user_id)."""
    render_sidebar_logo()
    st.sidebar.header("🔐 Expert Mode Authentication")
    
    if "expert_authenticated" in st.session_state and st.session_state.expert_authenticated:
...
def get_pro_user_selections():
    """Get user selections for Expert mode. Returns dict with user_id and language_code, or None."""
    ...
    language_code = "Scientific_Name" if selected_language == "Scientific Names" else LANGUAGE_MAPPING[selected_language]
